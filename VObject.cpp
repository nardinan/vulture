            ...
            while ((compare.vcmpcase(string, CSTRSIZE("[/OBJECT]")) != 0) && (!feof(configurationfile))) {
                if (compare.vcmpcase(string, CSTRSIZE("[OBJECT]")) != 0) {
                    if (compare.vcmpcase(string, CSTRSIZE("ID")) == 0) {
                        if (strings.vsscanf(string, '=', "sd", &name, &objectID) != 0) LOG_ERROR("Unable to edit key %s", string);
                    ...
                    } else if (compare.vcmpcase(string, CSTRSIZE("NOBJECT")) == 0) objectID = NobjectID++;
                    ...
                    } else if (compare.vcmpcase(string, CSTRSIZE("#")) != 0) LOG_ERROR("[Cobject] %s (unkown command)", string);
                }
                if (name) {
                    pvfree(name);
                    name = NULL;
    ...
    if (skeleton) {
        if (fgets(string, _DEFAULT_MAXD_LEN, skeleton)) {
            while ((compare.vcmpcase(string, CSTRSIZE("[/OBJECT]")) != 0) && (!feof(skeleton))) {
                if (compare.vcmpcase(string, CSTRSIZE("[OBJECT]")) == 0) {
                    if (load(skeleton) > 0) LOG_ERROR("Unable to load OBJECT'S shell");
                } else if (compare.vcmpcase(string, CSTRSIZE("#")) != 0) LOG_ERROR("[Cobject] %s (unknown command)", string);
...
int Cobject::save(FILE *configurationfile) {
    if (configurationfile) {
        ...
        if (descriptions.save(configurationfile) > 0) LOG_ERROR("Unable to save DESCRIPTIONS of Object %d", objectID);
        fputc('\n', configurationfile);
        if (logics.save(configurationfile) > 0) LOG_ERROR("Unable to save LOGICS of Object %d", objectID);
...
int Cobject::copy(Cobject *object) {
    ...
    if (logics.copy(object->logics) > 0) LOG_ERROR("Unable to copy LOGICS of Object %d", objectID);
    if (descriptions.copy(object->descriptions) > 0) LOG_ERROR("Unable to copy DESCRIPTIONS of Object %d", objectID);
    return 0;
}
